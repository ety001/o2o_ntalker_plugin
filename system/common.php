 ...
 */
function syn_topic_title($title_name,$type)
{
	$data = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."topic_title where name = '".$title_name."' and type = ".$type);
	if(!$data)
	{
		$data = array("name"=>$title_name,"type"=>$type);
...
function syn_supplier_locationcount($supplier_locationinfo)
{
	...
	$supplier_locationinfo['avg_point'] = floatval($GLOBALS['db']->getOne("select avg(point) from ".DB_PREFIX."supplier_location_dp where status = 1 and supplier_location_id = ".$supplier_locationinfo['id'])); 	
...
function im_key($user_id){
	$sitekey = app_conf('sitekey');
	return md5($user_id.$sitekey);
}
?>